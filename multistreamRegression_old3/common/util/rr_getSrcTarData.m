...
% D_src = datasample(D_partSS(1:6,:),numS,2,'Replace',true,'Weights');
% get target data
D_tar = datasample(D_partTT(1:6,:),numS*9,2,'Replace',true,'Weights',D_partTT(7,:));
% D_tar = datasample(D_partTT(1:6,:),numS*2,2,'Replace',true,'Weights');
if j == 1
...
tarData.input = dataDriftTar{1}(1:end-1,:);
tarData.output = dataDriftTar{1}(end,:);

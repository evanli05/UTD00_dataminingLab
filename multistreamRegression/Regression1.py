...
class Regression(object):
    ...
    def cal_weight(data_matrix):
        ...
        return weight
    @staticmethod
    def cal_weight_srctrg(data_matrix,target_matrix):
        """
        weight here is the probability density function p.d.f value for each data instance.
        :param data_matrix: M * N matrix where M - number of instances, N- number of features
        :return: a list of weight for each instance in the data matrix
        ...
        weight = []
        m, n = data_matrix.shape
        covariance, mean = Regression.get_covariance(target_matrix)
        for i in range(m):
            p = multivariate_normal.pdf(data_matrix[i, :], mean.tolist()[0], covariance, allow_singular=True)
            weight.append(p)
        return weight
    ...
    @staticmethod
    def get_est_mean(m, data_matrix,target_matrix):
        """
        ...
        weight_target = Regression.cal_weight_srctrg(matrix, target_matrix[:,:-1])
        m_yy = m[0, 0]
        m_yx1 = m[0, 1:]
        mu = []
    ...
    @staticmethod
    def get_predictY(m, data_matrix,target_matrix):
        """
        :param m: a row matrix contains current parameter values corresponding to [Y, x1,x2,...,xn, 1]
        :param data_matrix: M*(N+1) matrix, M - number of instances, N - number of features, 
        the last column is target value
        :return: mu - MLE estimate value of y for each data point.
        """
        matrix = target_matrix[:, :-1]
        ycol = data_matrix[:, -1]
        m_, n_ = target_matrix.shape
        weight_trg = Regression.cal_weight(target_matrix[:,:-1])
        weight_trgsrc = Regression.cal_weight_srctrg(target_matrix[:,:-1], data_matrix[:,:-1])
        m_yy = m[0, 0]
        m_yx1 = m[0, 1:]
        mu = []
        ...
        for i in range(m_):
            sample = matrix[i, :]
            w = weight_trgsrc[i]/weight_trg[i]
            if w == float("inf"):
                if m_yy == 0:
                    m_yy = 1e-10
                tmp = sample.tolist()[0]
    ...
    @staticmethod
    def update_m(m, step, decay, speed, data_matrix,target_matrix, estimated_mean=None, app_gradient=None,
        ...
        if estimated_mean is None:
            est_mean, mu = Regression.get_est_mean(m, data_matrix,target_matrix)
        else:
        ...
        lower_bound = Regression.get_m_yy_lower_bound(data_matrix,target_matrix)
        while m[0, 0] < lower_bound:
            m[0, 0] += abs(diff_m_yy)/2.0
        return m

    @staticmethod
    def get_m_yy_lower_bound(data_matrix,target_matrix):
        """
        ...
        y_col = data_matrix[:, -1]
        weight = Regression.cal_weight(matrix)
        weight1 = Regression.cal_weight_srctrg(matrix,target_matrix[:,:-1])
        _, base_variance = Regression.get_dist_base(y_col)
        lower_bound = -1 / (2 * base_variance * (weight[0]/weight1[0]))
        for i in range(1, len(weight)):
            temp = -1.0 / (2 * base_variance * (weight[i]/weight1[i]))
            if temp > lower_bound:
                lower_bound = temp
        return lower_bound

    @staticmethod
    def start(init_m, data_matrix,target_matrix, stop_thd, rate_initial, decay_tune, iteration=1000):
        """
        ...
        :return: 
        """
        print data_matrix.shape, target_matrix.shape
        n = 0
        ...
        while True:
            ...
            if n > iteration:
                return best_m
            estimated_mean, mu = Regression.get_est_mean(current_m, data_matrix,target_matrix)
...
#     matrix_ = Regression.read_csv('../pm2.5_srcFile1.csv', size=1000)
...
#     target_ = Regression.read_csv('../pm2.5_trgFile1.csv', size=1000)
#     best_m_ = Regression.start(
#         m0_, matrix_,target_, stop_thd=stopThd, rate_initial=rateInitial, decay_tune=decayTune, iteration=iteration)
#     end_time = time.clock()
#     print "Execution time for %d iterations is: %s min" % (
#         iteration, (end_time-start_time)/60.0)
#     print "train done"
#     #target_ = Regression.read_csv('../GGtarget', size=None)
#     #target_ = Regression.read_csv('../pm2.5_trgFile.csv', size=1000)
#     predict_y = Regression.get_predictY(best_m_, matrix_, target_)
#     true_y = Regression.get_true_y(target_)
#     trr = Regression.get_prediction_error(predict_y, true_y)
#     print "src test 1000_pm2.5adddrift:", trr